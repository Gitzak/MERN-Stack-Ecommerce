...
import { getAllOrders } from "../../../api/statisticsApi";
...
export const SalesChart = () => {
    ...
    const buttons = [
        <ToggleButton value="one" key="one" onClick={() => setTimePeriod("Daily")}>
            Daily
        </ToggleButton>,
        <ToggleButton value="two" key="two" onClick={() => setTimePeriod("Weekly")}>
            Weekly
        </ToggleButton>,
        <ToggleButton value="three" key="three" onClick={() => setTimePeriod("Monthly")}>
            Monthly
        </ToggleButton>,
        <ToggleButton value="four" key="four" onClick={() => setTimePeriod("Yearly")}>
            Yearly
        </ToggleButton>,
    ];
    ...
    const xAxisMin = Math.min(...graphData.map((item) => new Date(item.date).getTime()));
    const xAxisMax = Math.max(...graphData.map((item) => new Date(item.date).getTime()));
    ...
    return (
        <React.Fragment>
            ...
            <ToggleButtonGroup color="primary" value={alignment} exclusive onChange={handleChange} aria-label="Platform" sx={{ alignSelf: "center" }}>
                {buttons}
            </ToggleButtonGroup>
            <ResponsiveContainer>
                <LineChart
                    ...
                    <XAxis dataKey="date" stroke={theme.palette.text.secondary} style={theme.typography.body2} tickFormatter={formatXAxis} domain={[xAxisMin, xAxisMax]} />

                    <YAxis stroke={theme.palette.text.secondary} style={theme.typography.body2} domain={[yAxisMin, yAxisMax]}>
                        <Label
                            angle={270}
                            position="left"
                            style={{
